 ...
 * R0.01.02 2019/08/18 YJP/S.Kohno               行数ごとの高さ設定をStyle属性ではなくCSSクラスを用いるよう修正
 * [END OF MODIFICATION HISTORY]
 * ==========================================================================
 */
public class GnomesCTagInputTextArea extends GnomesCTagBase {
    ...
    /** 辞書：行数指定（Bean）**/
    private static final String INFO_INPUT_ROWS = "input_rows";

    /** 辞書：行数指定（数値）**/
    private static final String INFO_INPUT_ROWS_VALUE = "input_rows_value";
    ...
    /** 辞書：テキストエリア参照用名称 **/
    private static final String INFO_INPUT_TEXT_AREA_NAME = "input_text_area_name";
    ...
    /** 辞書：かな入力有りか否か **/
    private static final String INFO_INPUT_KANA = "input_kana";
    ...
    /** 入力値最大文字数確認の処理 スクリプト */
    private static final String INFO_ON_BLUR_EVENT = "delTextareaValMaxlengthOver";

    /** 行ごとの高さCSSクラス名（管理端末） */
    private static final String CTAG_TEXTAREA_HEIGHT_MANAGE_ROW = "ctag-textarea-height-manage-row-";

    /** 行ごとの高さCSSクラス名（工程端末） */
    private static final String CTAG_TEXTAREA_HEIGHT_OPERATE_ROW = "ctag-textarea-height-operate-row-";

    /** 管理端末の表示エリア高さ基底 */
//    private static final long TEXT_AREA_HEIGHT_CLIENT = 28;

    /** 管理端末の表示エリア高さ（追加部） */
//    private static final long TEXT_AREA_HEIGHT_ADD_CLIENT = 12;

    /** 工程端末の表示エリア高さ基底 */
//    private static final long TEXT_AREA_HEIGHT_PANECON = 34;

    /** 工程端末の表示エリア高さ（追加部） */
//    private static final long TEXT_AREA_HEIGHT_ADD_PANECON = 17;
    ...
    public int doStartTag() throws JspException {
        ...
        try {
            ...
            // ラベル辞書取得
            Map<String,Object> mapInfo = dict.getTextAreaInfo(this.dictId);
            ...
            // inputRows:行数
            String strInputRows = "1";
            // 行数指定（Bean）
            String labelInputRows = (String)mapInfo.get(INFO_INPUT_ROWS);
            // 行数指定（数値）
            String labelInputRowsValue = (String)mapInfo.get(INFO_INPUT_ROWS_VALUE);

            // 行数指定（Bean）が設定されている場合
            if (!StringUtil.isNullOrEmpty(labelInputRows)) {
                strInputRows = (String) getData(clsBean, bean, labelInputRows);
                if (StringUtil.isNullOrEmpty(strInputRows)){
                    strInputRows ="1";
                }
            }
            // 行数指定（数値）が設定されている場合
            else if (!StringUtil.isNullOrEmpty(labelInputRowsValue)) {
            	strInputRows = labelInputRowsValue;
            }
            ...
            if (isPanecon) {
                // 工程端末の場合
                heightClassName = " " + CTAG_TEXTAREA_HEIGHT_OPERATE_ROW + strInputRows;
            } else {
                // 管理端末の場合
                heightClassName = " " + CTAG_TEXTAREA_HEIGHT_MANAGE_ROW + strInputRows;
            }
            ...
            if ( labelRourceId != null) {
                ...
                // 左ラベルの出力
                out.print("  <div class=\"common-header-col-title" + heightClassName + strLabelClassName + "\">"
                + strLabel + "</div>");
            ...
            String labelInputTextArea = (String)mapInfo.get(INFO_PARAM_NAME);
            String strInputTextArea = (String) responseContext
                    .getResponseFormBean(bean, labelInputTextArea,
                            (String) mapInfo.get(INFO_INPUT_TEXT_AREA_NAME));
            if(StringUtil.isNullOrEmpty(strInputTextArea)){
                    strInputTextArea ="";
            }

            // inputTextName:input name="inputTextAreaName"
            String strInputTextAreaName = (String)mapInfo.get(INFO_INPUT_TEXT_AREA_NAME);
            if(StringUtil.isNullOrEmpty(strInputTextAreaName)){
                strInputTextAreaName = "";
            } else {
                strInputTextAreaName = " name=\"" + strInputTextAreaName + "\"";
            }
            ...
            // かな入力有りか否か＝無しの場合
            if(!StringUtil.isNullOrEmpty((String)mapInfo.get(INFO_INPUT_KANA))){
                strInputClass = strInputClass + " no-conversion";
            }
            ...
            // 最大文字数確認の処理
            String onblur = "";
            onblur = " onblur=\"" + INFO_ON_BLUR_EVENT + "('" + (String) mapInfo.get(INFO_INPUT_TEXT_AREA_NAME) + "');\"";

            out.print( "<div class=\"" + dataClassName + " common-input common-header-col-data-txtarea" + heightClassName + addClass + "\">");

            out.print( "<textarea " + strInputTextAreaName +" rows=\"" +  strInputRows + "\" " +
                strInputType + tabIndex + strMaxLength + " class=\"common-table-input-size-max" + strInputClass + strInputAddStyle + "\""+ onchange + onblur + ">" +
                this.getStringEscapeHtmlValue(strInputTextArea) + "</textarea></div>\n");
