 ...
 */
@Dependent
public class HistCertificationDao extends BaseDao implements Serializable {
    ...
    @TraceMonitor
    @ErrorHandling
    public void insert(HistCertification item) {
        if (item != null) {
            // 監査証跡情報を登録
            item.setReq(req);
            // 登録
    ...
    @TraceMonitor
    @ErrorHandling
    public void insert(HistCertification item, EntityManager eml) {
        if (item != null) {
