...
class GnomesCTagTableDateFormatTest {
  ...
  void test_format_date_by_dataType_input(int dataType) throws Exception {
    ...
    // inputDataType実行
    ReflectionTestUtils.invokeMethod(cTagTable, "inputDataType", out, null, null, mapColInfo, null,
        null, 1, false);
  ...
  void test_format_date_by_dataType_output(int dataType) throws Exception {
    // @ParameterizedTestによって@ValueSourceの引数の数だけテストメソッドが呼ばれる
    setupMockGetData(dataType);
    // outputDataType実行
    ReflectionTestUtils.invokeMethod(cTagTable, "outputDataType", out, null, null, null, mapColInfo,
        null, null, 1, false);

    // InOrder.verifyでJspWriter.outが呼ばれることを検証
    // カスタムタグ自体の検証ではないのでフォーマット後日付部分のみ対象
    inOrder.verify(out, times(1))
        .print(">" + TestCTagUtil.convertDateToString(dataType, beforeDate) + "</div>\n");
  }

  private void setupMockGetData(int dataType) throws Exception {
    // doReturn第1引数はinputDataBase、第2引数はoutputDataBase内のgetDataの戻り値
    doReturn(dataType, beforeDate).when(cTagTable).getData(any(), any(), any());
  }
