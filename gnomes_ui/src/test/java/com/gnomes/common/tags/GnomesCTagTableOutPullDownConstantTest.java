...
class GnomesCTagTableOutPullDownConstantTest {
    ...
    @Test
    @DisplayName("プルダウン出力（コード等定数）：タグ名が未設定の場合selectタグのname属性が出力されないことの確認")
    void test_outPullDownConstant_tagName_empty() throws Exception {
        mapInfo("test_pulldown", "", 0, false);
        setupMockReturnBool(false, false);
        ReflectionTestUtils.invokeMethod(cTagTable, "outPullDownConstant", out, TD_STYLE, SELECT_STYLE, mapColInfo, null, null, ROW_COUNT, false);
    ...
    @Test
    @DisplayName("プルダウン出力（コード等定数）：FormBeanから選択値を取得かつ候補リストに選択値が存在する場合選択値が選択された状態で出力されることの確認")
    void test_outPullDownConstant_select_true() throws Exception {
    ...
    @Test
    @DisplayName("プルダウン出力（コード等定数）：差し替え用選択名パラメータ名が設定されている場合選択値表示名が差し替えられた状態で出力される")
    void test_outPullDownConstant_select_true_replacement() throws Exception {
        mapInfo("test_pulldown", "test_tag", 2, false);
        setupMockReturnBool(true, false);
        ReflectionTestUtils.invokeMethod(cTagTable, "outPullDownConstant", out, TD_STYLE, SELECT_STYLE, mapColInfo, null, null, ROW_COUNT, false);
    ...
    @Test
    @DisplayName("プルダウン出力（コード等定数）：非表示項目に設定されている場合tdタグのstyle属性に'display:none'が出力されることの確認")
    void test_outPullDownConstant_hidden_true() throws Exception {
        mapInfo("test_pulldown", "test_tag", 1, true);
        setupMockReturnBool(true, false);
        ReflectionTestUtils.invokeMethod(cTagTable, "outPullDownConstant", out, TD_STYLE, SELECT_STYLE, mapColInfo, null, null, ROW_COUNT, false);
    ...
    @Test
    @DisplayName("プルダウン出力（コード等定数）：readonlyの場合選択データを隠し項目として出力しプルダウン部はdisabledとして出力されることの確認")
    void test_outPullDownConstant_isInputReadOnlyParam_true() throws Exception {
        mapInfo("test_pulldown", "test_tag", 1, false);
    ...
    @Test
    @DisplayName("プルダウン出力（コード等定数）：スタイルクラスがNullの場合tdタグのclass属性が出力されないことの確認")
    void test_outPullDownConstant_styleClass_null() throws Exception {
        mapInfo(null, "test_tag", 1, false);
        setupMockReturnBool(true, false);
        ReflectionTestUtils.invokeMethod(cTagTable, "outPullDownConstant", out, TD_STYLE, SELECT_STYLE, mapColInfo, null, null, ROW_COUNT, false);
    ...
    @Test
    @DisplayName("プルダウン出力（コード等定数）：tdのスタイルがNullの場合tdタグのstyle属性が出力されないことの確認")
    void test_outPullDownConstant_td_style_null() throws Exception {
        mapInfo("test_pulldown", "test_tag", 1, false);
        setupMockReturnBool(true, false);
        ReflectionTestUtils.invokeMethod(cTagTable, "outPullDownConstant", out, null, SELECT_STYLE, mapColInfo, null, null, ROW_COUNT, false);
    ...
    @Test
    @DisplayName("プルダウン出力（コード等定数）：selectのスタイルがNullの場合selectタグのstyle属性がtdのスタイルで適用されることの確認")
    void test_outPullDownConstant_select_style_null() throws Exception {
        mapInfo("test_pulldown", "test_tag", 1, false);
        setupMockReturnBool(true, false);
        ReflectionTestUtils.invokeMethod(cTagTable, "outPullDownConstant", out, TD_STYLE, null, mapColInfo, null, null, ROW_COUNT, false);
    ...
    @Test
    @DisplayName("プルダウン出力（コード等定数）：空白有りの場合プルダウンに空白の選択肢が出力されることの確認")
    void test_outPullDownConstant_defaultSpace_true() throws Exception {
        mapInfo("test_pulldown", "test_tag", 1, false);
        setupMockReturnBool(true, false);
        // 空白有り
        ReflectionTestUtils.invokeMethod(cTagTable, "outPullDownConstant", out, TD_STYLE, SELECT_STYLE, mapColInfo, null, null, ROW_COUNT, true);
    ...
    private void mapInfo(String styleClass, String tagName, int paramNameLength, boolean isHidden) {
