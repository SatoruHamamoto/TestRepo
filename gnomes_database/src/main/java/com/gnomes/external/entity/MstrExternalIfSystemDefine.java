...
@Table(name = "mstr_external_if_system_define")
@NamedQueries({
        @NamedQuery(name = "MstrExternalIfSystemDefine.findAll", query = "SELECT p FROM MstrExternalIfSystemDefine p"),
        @NamedQuery(name = "MstrExternalIfSystemDefine.findByPK", query = "SELECT p FROM MstrExternalIfSystemDefine p WHERE p.external_if_system_define_key = :external_if_system_define_key")
})
public class MstrExternalIfSystemDefine extends BaseEntity implements Serializable {

    /** テーブル名 */
    public static final String TABLE_NAME = "mstr_external_if_system_define";

    /** 外部I/Fシステム定義キー */
    public static final String COLUMN_NAME_EXTERNAL_IF_SYSTEM_DEFINE_KEY = "external_if_system_define_key";
    ...
    /** 外部I/Fシステム定義キー */
    private String external_if_system_define_key;
    /** nk外部I/F対象システムコード */
    ...
    /**
     * Zm130外部I/Fシステム定義マスタエンティティ コンストラクタ
     */
    public MstrExternalIfSystemDefine() {
    }

    /**
     * Zm130外部I/Fシステム定義マスタエンティティ コンストラクタ
     * @param external_if_system_define_key 外部I/Fシステム定義キー
     * @param external_if_target_code nk外部I/F対象システムコード
     * @param definition_code nk定義項目コード
     * @param version 更新バージョン
     */
    public MstrExternalIfSystemDefine(String external_if_system_define_key, String external_if_target_code, String definition_code, int version) {
        this.external_if_system_define_key = external_if_system_define_key;
    ...
    /**
     * 外部I/Fシステム定義キーを取得
     * @return 外部I/Fシステム定義キー
     */
    @Id
    @Column(nullable = false, length = 38)
    public String getExternal_if_system_define_key() {
        return this.external_if_system_define_key;
    }

    /**
     * 外部I/Fシステム定義キーを設定
     * @param external_if_system_define_key 外部I/Fシステム定義キー (null不可)
     */
    public void setExternal_if_system_define_key(String external_if_system_define_key) {
        this.external_if_system_define_key = external_if_system_define_key;
    }

    /**
     * nk外部I/F対象システムコードを取得
     * @return nk外部I/F対象システムコード
     */
    ...
    /**
     * nk外部I/F対象システムコードを設定
     * @param external_if_target_code nk外部I/F対象システムコード (null不可)
     */
